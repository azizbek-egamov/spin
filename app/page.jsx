...
export default function Home() {
  ...
  const fallbackPrizes = [
    { id: 1, name: "1,000,000 so'm" },
    { id: 2, name: "500,000 so'm" },
    { id: 3, name: "iPhone 15" },
    { id: 4, name: "0 so'm" },
    { id: 5, name: "100,000 so'm" },
    { id: 6, name: "Velosiped" },
    { id: 7, name: "Televizor" },
    { id: 8, name: "Omadsiz" },
  ]
  ...
  useEffect(() => {
    ...
    const fetchPrizes = async () => {
      try {
        ...
        try {
          const response = await fetch("https://pyco1.pythonanywhere.com/gifts/", {
  ...
  const handleWin = async (giftId, userId) => {
    try {
      ...
      // Try to send the win data to the provided API
      try {
        const response = await fetch("https://pyco1.pythonanywhere.com/gift-add/", {
          method: "POST",
          headers: {
